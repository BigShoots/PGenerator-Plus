...
- **Display profile creation** — measured Windows MHC2 profiles and KDE HDR cLUT profiles can be built in the browser. Advanced format controls select ICC v2.2 or v4.4 and expose the v4.4 CICP primaries, transfer, matrix and range code points. Calibration can be stored in VCGT, incorporated into the ICC without VCGT, or omitted for a display already calibrated internally. KDE HDR defaults to Rec. 2020/PQ/full-range CICP and OLED near-black cLUT stabilization. With Patch Companion connected, completed builds and Profile History can install and apply an ICC directly to the display showing the patches. On Windows, Profile Loader also enables the per-user profile list shown as "Use my settings for this device" in Color Management.
