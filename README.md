...
- **Display profile creation** — measured Windows MHC2 profiles and KDE HDR cLUT profiles can be built in the browser. Advanced format controls select ICC v2.2 or v4.4 and expose the v4.4 CICP primaries, transfer, matrix and range code points. KDE HDR defaults to Rec. 2020/PQ/full-range CICP, VCGT handling, and OLED near-black cLUT stabilization. With Patch Companion connected, completed builds and Profile History can install and apply an ICC directly to the display showing the patches.
...
- The current image/runtime bundles an ArgyllCMS `3.5.0` armhf runtime. Its `colprof` comes from the PGenerator+ ICC v4.4/CICP fork so KDE HDR profiles can be written with a v4.4 header and CICP `9-16-0-1`. The source remains in the separate ArgyllCMS fork rather than this repository.
