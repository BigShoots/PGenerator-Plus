...
// HLG stays on the container too. DV/SDR fall through to target_key.
const lines=(src.match(/my \$solve_key=[^\n;]*;/g)||[]);
assert(lines.length>=2,'expected at least two solve_key assignments');
lines.forEach((line,i)=>{
  // solve_key must reference either target_key (DV/SDR) or container_key
  // (HDR10/HLG saturation stability).
