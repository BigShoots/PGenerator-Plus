...
assert(
	    ...
	    /function meterLgHdrAutoCalDdcArrayIre\(slot\)\{\s*return slot;\s*\}/.test(source) &&
	    ...
	    !autoCalTargetLuminanceSource.includes('target_gamma_linear($signal,"2.2","sdr")'),
		  'HDR20 AutoCal should use code-derived HDR weighted patches with identity LG HDR20 DDC slots, calibrate 100% instead of SDR-style paired 99/100, use the supplied AutoCal target curve, and avoid luma-only HDR100 moves while chroma is still high'
	);
assert(
