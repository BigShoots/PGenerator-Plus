...
assert(
	    source.includes('const METER_LG_GREY_HDR_AUTOCAL_SLOTS=[100,94.98,89.95,84.93,79.91,69.86,59.82,50.23,40.18,30.14,25.11,20.09,15.07,10.05,6.85,5.02,4.11,2.74,1.83,1.37];') &&
		    ...
		    !source.includes('if(Math.abs(value-94.98)<0.02) return 100;') &&
		    !source.includes('if(Math.abs(value-89.95)<0.02) return 94.98;') &&
		    !source.includes('if(Math.abs(value-84.93)<0.02) return 89.95;') &&
		    !source.includes('if(Math.abs(value-79.91)<0.02) return 84.93;') &&
		    ...
		    autocalWorkerSource.includes('return $ire+0;') &&
		    !autocalWorkerSource.includes('return 100 if(abs($value-94.98) < 0.02);') &&
		    !autocalWorkerSource.includes('return 94.98 if(abs($value-89.95) < 0.02);') &&
		    !autocalWorkerSource.includes('return 89.95 if(abs($value-84.93) < 0.02);') &&
		    !autocalWorkerSource.includes('return 84.93 if(abs($value-79.91) < 0.02);') &&
	    autocalWorkerSource.includes('sub hdr20_top_white_chroma_priority_needed') &&
	    autocalWorkerSource.includes('sub hdr20_body_force_luma_clamp_needed') &&
	    autocalWorkerSource.includes('full_ddc_spine_anchor_force_luma_clamp') &&
	    ...
	    !autoCalTargetLuminanceSource.includes('target_gamma_linear($signal,"2.2","sdr")'),
			  'HDR20 AutoCal should use code-derived HDR weighted patches with direct LG HDR20 DDC slots, calibrate 100% instead of SDR-style paired 99/100, use the supplied AutoCal target curve, and avoid luma-only HDR100 moves while chroma is still high'
		);
assert(
