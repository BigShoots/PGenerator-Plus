   ...
   Dolby Vision transport notes (for future maintainers):
   - Two transports exist (variables.pm pg_dv_transport_* helpers, WebUI "Transport" select): `standard` (sink-led: DV tunneled in RGB 8-bit with embedded RPU metadata; PROVEN working on a full-DV TV) and `lldv` (source-led low-latency: YCbCr 4:2:2 12-bit with the LL bit in the Dolby VSIF). CAUTION: the renderer's LLDV baseband is PQ YCbCr; a strict LLDV sink expects IPTPQc2 (ICtCp) and will show a flat green/magenta wash. If a display shows solid green in LLDV but standard DV works elsewhere, this is why - producing a true LLDV signal needs an RGB->IPTPQc2 path in the DV shader (not yet implemented).
   - The DV signal mode is gated on the TV advertising the Dolby VSVDB in its EDID (`has_dv` in `webui_capabilities_json`). A `dv_edid_override` conf knob ("DV Capability: Force supported") that bypassed this existed briefly and was removed in 2.7.3; a stale `dv_edid_override=` key in an existing conf is inert. Note some TVs only expose the VSVDB (or working DV) on specific HDMI inputs.
   - NEVER signal GCP deep colour for YCbCr 4:2:2 (the kernel patch once did, to make displays report 10/12-bit): 4:2:2 is a fixed 36-bit container at 1x pixel clock and spec-following sinks derive a bogus refresh from the CD field (4K60 shown as "40Hz" at CD=12 / "48Hz" at CD=10) and their HDR/DV decode breaks. Fixed in the patch as of commit e2c8ec98.
   - A TV claiming "Dolby Vision" while the source sends none can be the TV's own per-input latch: verify the wire first via the HDMI RAM packet config (`grep RAM_PACKET_CONFIG /sys/kernel/debug/dri/1/hdmi0_regs`; bit1=vendor/DV VSIF, bit2=AVI, bit3=SPD, bit7=DRM/HDR) instead of trusting the TV OSD. In SDR the renderer transmits an explicit EOTF=0 DRM infoframe (commit 80d29046) so latched sinks exit HDR/DV.
