   ...
   - `dv_edid_override=1` (WebUI "DV Capability: Force supported") unlocks the DV signal mode for displays that decode DV but do not advertise the Dolby VSVDB in their EDID. Some TVs only expose the VSVDB (or working DV) on specific HDMI inputs.
