...
sub webui_meter_series_start (@) {
 ...
 if($type eq "greyscale") {
  ...
  @steps=map {
   my $v=$_;
   my $c=0;
    # Calman LG DV greyscale steps are nominal equal-code RGB patches.
    ...
    # shifts the patches far above Calman's step table and breaks both the
  ...
  my $span_code=$patch_limited?219:255;
    ...
    # Calman DV color runs are target-gamut driven (LG workflow uses D65_DCI /
    ...
    foreach my $sat (
     ...
     my ($name,$series_color,$r_mix,$g_mix,$b_mix)=@$sat;
      # Calman's full-saturation DV patches are direct RGB percentages with the
...
function meterChartTrackingLuminance(v,clipPeak,Lw,Lb){
 ...
 if(meterChartIsDv()){
  // Calman LG Dolby Vision chart targets follow ST.2084/PQ from the emitted
