...
#define DRM_DEVICE0 "/dev/dri/card0"
#define DRM_DEVICE1 "/dev/dri/card1"
...
int main(int argc, char *argv[])
{
    ...
    /* Open DRM device — try both cards, use whichever has a display */
    const char *drm_devices[] = { DRM_DEVICE0, DRM_DEVICE1, NULL };
    struct drm_state st;
    int found_display = 0;

    for (int di = 0; drm_devices[di] && !found_display; di++) {
        drm_fd = open(drm_devices[di], O_RDWR | O_CLOEXEC);
        if (drm_fd < 0) {
            logmsg("Cannot open %s: %s\n", drm_devices[di], strerror(errno));
            continue;
        }
        logmsg("Trying %s\n", drm_devices[di]);
        ...
        if (become_drm_master() < 0) {
            logmsg("Cannot become DRM master on %s\n", drm_devices[di]);
        }
        ...
        if (drm_find_connector(&st) == 0) {
            ...
            logmsg("No display on %s, trying next...\n", drm_devices[di]);
            drm_ioctl(DRM_IOCTL_DROP_MASTER, NULL);
            close(drm_fd);
            drm_fd = -1;
        }
    ...
    if (!found_display) {
        logmsg("No connected display found on any DRM device\n");
        return 1;
    }
