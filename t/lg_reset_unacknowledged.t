...
like($result->{message},qr/did not acknowledge|timeout/i,'the missing acknowledgement is reported, not silently absorbed');
...
ok(!lg_3d_lut_upload_retryable_failure({ status=>'error', error_code=>'lg-calibration-end-unconfirmed', message=>'LG TV did not confirm calibration mode end after the 3D LUT write.', upload_write_verified=>JSON::PP::true }),
 'an accepted write with unconfirmed CAL_END is never retried on a fresh socket');
