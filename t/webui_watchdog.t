...
# A full-size healthy response stamps the daemon PID, and the stamp
# suppresses every later probe for the same daemon instance — a re-run in a
# mode that would otherwise log a connect error must stay silent. A new PID
# invalidates the stamp and the probe runs (and logs) again.
{
    ...
    ok(-f $stamp, "a healthy full-size probe writes the root-ok stamp");
    {
        ...
        is(system("sh", $script), 0, "watchdog exits cleanly when the stamp skips the probe");
    }
    my $stamp_text = "";
    ...
    is($stamp_text, "", "a stamped daemon instance is not re-probed");
