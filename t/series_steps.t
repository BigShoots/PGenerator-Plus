...
sub run_normalizer {
 my ($steps,$generation,$mode,$requested_count,$trigger)=@_;
 ...
 my $pid=open3(undef,my $stdout,$error,$python,$helper,"normalize",$path,
  $generation,$mode,$requested_count,$trigger,"1931_2");
 local $/;
 my $output=<$stdout>||"";
 my $errors=<$error>||"";
 waitpid($pid,0);
...
my ($status,$output,$errors)=run_normalizer($steps,7,"aa",3,1);
...
is_deeply([splice(@fields,0,4)],["pgen-series-steps","2","7","2"],
 "stream header pins schema, generation, and count");
is(scalar(@fields),34,"each step has seventeen fixed-order fields");
my @first=splice(@fields,0,17);
...
is($first[13],1,"inactive step executes one physical sample");
is($first[14],"true","autocal white-reference marker is normalized");
is_deeply(decode_json($first[15]),$steps->[0],
 "full normalized step preserves original metadata");
my $first_metadata=decode_json('{'.$first[16].'}');
...
is($second[13],3,"active step executes the ingress numeric count");
...
my ($large_status,$large_output,$large_errors)=run_normalizer(\@large,19,"aaa",5,1);
...
is(scalar(@large_fields),4+17*1000+1,
