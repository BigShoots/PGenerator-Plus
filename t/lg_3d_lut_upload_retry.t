...
ok(lg_calibration_end_confirmed({type=>'error'},1,1),'a caller-fingerprinted tolerated DV rejection confirms CAL_END');
ok(!lg_calibration_end_confirmed({type=>'error'},1,0),'an unfingerprinted failed CAL_END remains unconfirmed');
ok(!lg_calibration_end_confirmed({type=>'response'},0,1),'tolerated alone cannot weaken an unacknowledged response');
...
($result,$calls)=run_scripted_upload(
 { status=>'error', error_code=>'lg-calibration-end-unconfirmed', upload_write_verified=>JSON::PP::true(), upload_verified=>JSON::PP::false(), message=>'write verified, close unconfirmed' },
 { status=>'ok', upload_verified=>JSON::PP::true(), message=>'must not run on a foreign socket' },
);
is($calls,1,'an accepted write with unconfirmed CAL_END is never retried on a foreign socket');
is($result->{error_code},'lg-calibration-end-unconfirmed','the stable close error survives the wrapper');
ok(!$result->{retry_exhausted},'the safety stop is not labelled retry exhaustion');
