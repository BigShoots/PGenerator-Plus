...
# C2-and-earlier Dolby Vision restores the hardware-era scoped Luna route.
# G5+ keeps the bare SSAP route that has verified readback on current sets.
reset_tv(mode => "dolbyHdrCinema");
$tv->{"on_write"}=sub {
 my ($route)=@_;
 $tv->{"mode"}="dolbyHdrCinemaBright" if($route eq "luna");
};
$r=lg_picture_mode_select({},"10.0.0.1","dolbyHdrCinemaBright",7,{platform_year=>2022},"hdmi2","dv");
ok($r->{"verified"},'C2 Dolby Vision scoped Luna write is verified by readback');
is($r->{"route"},'luna-scoped','C2 Dolby Vision uses the scoped Luna route');
my @legacy_writes=grep { $_->{"uri"}=~/setSystemSettings/ && $_->{"id"}=~/^set_picture_mode/ } @requests;
is(scalar(@legacy_writes),1,'C2 sends exactly one picture-mode write');
like($legacy_writes[0]{"uri"},qr/^luna:/,'C2 sends Luna first without an unproven bare SSAP attempt');
ok($legacy_writes[0]{"payload"}{"current_app"},'C2 scoped write targets the current app');
is($legacy_writes[0]{"payload"}{"dimension"}{"input"},'hdmi2','C2 scoped write carries the active HDMI input');

reset_tv(mode => "dolbyHdrCinema");
$r=lg_picture_mode_select({},"10.0.0.1","dolbyHdrCinemaBright",7,{platform_year=>2022},"","dv");
ok(!$r->{"verified"},'C2 Dolby Vision fails closed without a proven input scope');
is($r->{"blocked_reason"},'legacy-dv-input-scope-unavailable','the C2 fail-closed reason is explicit');
is(scalar(grep { $_->{"id"}=~/^set_picture_mode/ } @requests),0,'C2 sends no picture-mode write without input scope');

reset_tv(mode => "dolbyHdrCinema");
$tv->{"on_write"}=sub {
 my ($route)=@_;
 $tv->{"mode"}="dolbyHdrCinemaBright" if($route eq "ssap");
};
$r=lg_picture_mode_select({},"10.0.0.1","dolbyHdrCinemaBright",7,{platform_year=>2025},"hdmi2","dv");
ok($r->{"verified"},'G5 Dolby Vision bare route is verified by readback');
is($r->{"route"},'ssap','G5 Dolby Vision keeps the bare SSAP-first route');
my ($g5_write)=grep { $_->{"id"} eq "set_picture_mode_ssap" } @requests;
ok(ref($g5_write) eq 'HASH','G5 sends the public settings write');
ok(!exists($g5_write->{"payload"}{"dimension"}) && !exists($g5_write->{"payload"}{"current_app"}),'G5 public write remains unscoped');
...
is(lg_apply_all_inputs_generation_support({}),-1,'unclassified generations remain unknown');
{
 ...
 foreach my $model (sort(keys(%expected_series))) {
  my $generation=lg_generation_info({ modelName => $model },{},{});
  is($generation->{series},$expected_series{$model},"Perl generation parsing preserves $expected_series{$model}");
 }
 foreach my $case (
  ['OLED65C10AUA','C10'],
  ['OLED77C11AUA','C11'],
  ...
  my ($model,$series)=@{$case};
  my $generation=lg_generation_info({ modelName => $model },{},{});
  is($generation->{series},$series,"Perl generation parsing consumes the full $series token");
  isnt($generation->{generation_id},'lg2021_oled',"$series is not classified as the 2021 C1/G1 generation");
  ok(!$generation->{ddc_only_white_balance},"$series is not classified as pre-2022 DDC-only by the model token");
 }
}
...
is(lg_picture_mode_for_calibration('hdrEco'),'','HDR Auto Power Save is selectable but frozen from calibration');
is(lg_picture_mode_for_calibration('hdrPersonalized'),'','HDR Personalised Picture is selectable but frozen from calibration');
is(lg_picture_mode_for_calibration('dolbyVisionPersonalized'),'','DV Personalised Picture is selectable but frozen from calibration');
is(lg_picture_mode_for_calibration('dolbyHdrPersonalized'),'','the raw webOS DV Personalised token is also frozen from calibration');
is(lg_picture_mode_for_calibration('dolby_hdr_personalized'),'','the generic Dolby fallback cannot invent a Personalised cal mode');
{
...
like($lg_js,qr/\(\?:8\|9\|X\|1\)\(\?!\\d\)/,'the pre-2022 model detector requires C1/G1 to end before another digit');
my $pre2022_model_re=qr/OLED\d*(?:A|B|C|E|G|R|W|Z)(?:8|9|X|1)(?!\d)/;
ok('OLED65C1AUA'=~$pre2022_model_re,'a real C1 remains in the pre-2022 picture-mode map');
ok('OLED65CXPUA'=~$pre2022_model_re,'a real CX remains in the pre-2022 picture-mode map');
ok('OLED65C9PLA'=~$pre2022_model_re,'a real C9 remains in the pre-2022 picture-mode map');
ok('OLED65C2PUA'!~$pre2022_model_re,'a real C2 stays on the current-generation map');
ok('OLED83G54LW'!~$pre2022_model_re,'a real G5 stays on the current-generation map');
ok('OLED65C10AUA'!~$pre2022_model_re,'synthetic C10 is not misclassified as C1');
ok('OLED77C11AUA'!~$pre2022_model_re,'synthetic C11 is not misclassified as C1');
ok('OLED83G10AUA'!~$pre2022_model_re,'synthetic G10 is not misclassified as G1');
{
 local $/;
 open(my $hf,'<',$helper) or die "Unable to read $helper: $!";
 my $helper_source=<$hf>;
 close($hf);
 like($helper_source,qr/my \$hdr_picture_mode=.*technicolor/i,'raw hdrTechnicolor is included in the HDR reset detector');
}
