...
- Full AutoCal post-3D cleanup must not launch the committed-polish worker from post-polish verify alone. The post-3D worker should run only when post-series DDC adjustment is checked or committed polish is checked; verify can be passed through to those flows, but it is not a standalone trigger.
