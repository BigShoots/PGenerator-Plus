...
- 2026-05-26 EOTF chart checkbox regression: the normalized toggle must produce distinct plotted values. Normalized EOTF scales to the active peak; non-normalized keeps the absolute EOTF-domain value. Do not let gamma-style modes collapse both paths to `Y / peak`, and do not bypass normalization for PQ/DV absolute.
