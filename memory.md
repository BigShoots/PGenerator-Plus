...
- LG 26pt EOTF/luminance charts should not plot the synthetic/legal-white 100% reference point; use 99% as the top legal-white plot point. Keep 0% connected to 2.3%, but draw measured EOTF/luminance segments with the target transfer-curve shape instead of simple straight chords so the low-shadow connector follows the same visual trajectory as the rest of the curve. EOTF, luminance, and gamma charts should show lines only, no dots; the EOTF/luminance measured line should be thinner than the target line.
