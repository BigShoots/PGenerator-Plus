...
- 2026-05-25 Custom / D65 UI layout: the target colorspace field should stay compact unless Custom / D65 is selected. The white-point x/y row uses the parent `.has-whitepoint` class to widen the field only while those controls are visible, and the XYZ matrix wrapper must be hidden while disabled so other target colorspaces do not leave a blank gap.
- 2026-05-25 DV EOTF/target-gamma decision: do not add a duplicate EOTF selector to the Dolby Vision output card because the calibration card already exposes the target gamma/EOTF. Keep DV HDMI/config output EOTF pinned to ST.2084/PQ (`eotf=2`) when DV is selected. The calibration target default is map-mode dependent: DV Relative (Calibration, `dv_map_mode=2`) defaults to Gamma 2.2, DV Absolute (Mastering, `dv_map_mode=1`) defaults to ST.2084/PQ. Original PGenerator 1.6 Display Control did not expose an HDR EOTF selection for DV; it used DV/DOVI metadata and map mode, with SDR/HDR/HLG controlled through `is_hdr` plus EOTF (`2` PQ, `3` HLG).
- 2026-05-25 DV greyscale analysis rule: do not use `meterChartIsPq()` as the greyscale-analysis selector because DV transport is always HDR-ish. DV Relative (Calibration) must analyze/plot EOTF and gamma in Gamma 2.2 relative-luminance space unless the user explicitly selects ST.2084; DV Absolute (Mastering) must use ST.2084/PQ. DV Absolute greyscale series patches should be PQ-coded legal video levels, not linear/legal percent codes.
- 2026-05-25 DV Relative chart rule: EOTF and luminance measured traces should directly connect measured samples in DV Relative (`dv_map_mode=2`). Do not target-shape/densify the measured line there, because that can create a visible synthetic kink around normal sample points such as 75% even when the actual measured gamma and luminance-error values are smooth. Keep target-shaped measured interpolation available for other greyscale/headroom chart modes.
- 2026-05-25 luminance log chart rule: do not draw luminance log-scale target or measured curves from a fake black-origin point. Log axes cannot represent true 0 cd/m², so the luminance chart should begin the plotted line at the first positive target/measured sample while still showing black level and contrast metadata separately.
...
- Current full-DDC spine anchors are `109,20,40,60,80`, then the seeded pass runs downward from `105` to `15`, and the shadow-detail pass runs upward from `2.3,3,4,5,7,10`. 5% is not a full-DDC spine anchor.
- 5% must not use a hidden default low-shadow target-Y bias during initial greyscale. In-cal and committed post-series dE+Y need the same target luminance basis unless a future explicit config proves otherwise.
...
- LG 26pt EOTF/luminance charts should not plot the synthetic/legal-white 100% reference point; use 99% as the top legal-white plot point. Keep 0% connected to 2.3%, but draw measured EOTF/luminance segments and the nominal target curve with dense target-transfer-curve shaping instead of simple straight chords so the low-shadow connector follows the same visual trajectory as the rest of the curve. EOTF, luminance, gamma, and RGB balance charts should show lines only, no dots; the EOTF/luminance measured line should be thinner than the target line.
...
- 2026-05-26 Magic Wand hardware validation passed from restored post-3D snapshot `post-3d-before-magic-wand-clean` with artifact `tmp/magic-wand-phase4-highcap-20260526085344-20260526085344`: mean dE ITP+Y improved `1.4382 -> 0.5857`, max improved `5% 3.0979 -> 105% 0.9894`, 99/100 worst-side improved `2.225 -> 0.706`, 5% improved `3.098 -> 0.435`, and 85/90/95 improved `2.319/2.346/2.019 -> 0.542/0.374/0.421`. The high-end fix is a narrow Magic Wand luma cap increase for 85-95% when luma error is at least about 2.5%; do not generalize that cap to normal AutoCal.
...
- 2026-05-26 LG 26pt post-cal series order: read the hidden 100% legal-white reference first with a 3 second per-step delay, then read 0% for black/contrast, then measure upward from `2.3` through `109`. This is post-cal/report validation order only; it does not change AutoCal's internal calibration order or full-DDC spine anchors.
...
- 2026-05-26 EOTF chart checkbox regression: the normalized toggle must produce distinct plotted values. PQ/DV Absolute normalized EOTF scales PQ code values to the active peak, while unchecked keeps the absolute PQ code value. SDR/DV Relative normalized EOTF plots `Y / peak`, while unchecked plots inverse-EOTF signal space (Gamma/BT.1886/sRGB), so the checkbox has a visible and meaningful effect outside PQ without changing the luminance chart.
