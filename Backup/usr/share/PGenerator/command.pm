...
sub pattern_generator_start(@) {
 ...
 &get_hdmi_info();
 # Choose renderer based on max_bpc:
 #   max_bpc >= 10: pgeneratord_10bit — DRM dumb buffers with XRGB2101010,
 #                  native 10-bit precision, no OpenGL (Mesa vc4 can't do 10-bit)
 #   max_bpc <  10: PGeneratord — original OpenGL renderer with LD_PRELOAD fix
 my $max_bpc = int($pgenerator_conf{'max_bpc'} || 8);
 if ($max_bpc >= 10 && -x "/usr/sbin/pgeneratord_10bit") {
  $pattern_generator_active = "/usr/sbin/pgeneratord_10bit";
  &log("Starting 10-bit pattern generator (max_bpc=$max_bpc)");
  system("/usr/sbin/pgeneratord_10bit $w_s $h_s >/tmp/pgeneratord.log 2>&1 &");
 } else {
  $pattern_generator_active = $pattern_generator;
  ...
  system("LD_PRELOAD=/usr/lib/drm_override.so LD_LIBRARY_PATH=/usr/lib $pattern_generator $w_s $h_s >/tmp/pgeneratord.log 2>/tmp/drm_override.log &");
 }
...
sub pattern_generator_stop {
 ...
 # Kill whichever renderer is active (original or 10-bit)
 my $pg = $pattern_generator_active || $pattern_generator;
 &process_pid("$pg","kill");
 usleep(500000);
 while(($pid=&process_pid("$pg","get"))) {
  &process_pid("$pg","kill");
  usleep(500000);
 }
 # Also kill the other renderer just in case
 if ($pg ne $pattern_generator) {
  &process_pid("$pattern_generator","kill");
 }
 &process_pid("/usr/sbin/pgeneratord_10bit","kill");
 &clean_files();
}
