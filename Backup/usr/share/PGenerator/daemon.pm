   ...
   if ($connection == $server || $connection == $server_calman) {
    ...
    if($calman{$connection} && $key=~/(.*):(.*)/) {
     ...
     my $calman_save_setting = sub {
      ...
      # 8bpc, so pattern rendering stays 8-bit.  max_bpc only controls the
      # HDMI link depth (needed for HDR metadata at 10-bit).
     ...
     my $calman_apply = sub {
      if($calman_settings_dirty) {
       ...
       &log("Calman APPLY: PGeneratord restarted");
      }
     };
     #
     ...
     if($type =~/RGB_/) {
      ...
      &log("Calman PATTERN: type=$type raw=$el_cmd[0],$el_cmd[1],$el_cmd[2] bits_default=$bits_default calman_max=$calman_max target_max=$target_max");
      ...
      &log("Calman PATTERN: scaled=$r,$g,$b bg=$calman_bg quant=$pgenerator_conf{'rgb_quant_range'} max_bpc=$pgenerator_conf{'max_bpc'}");
      ...
      if($type =~/RGB_B/) {
       ...
       &log("Calman RGB_B: bg_raw=$el_cmd[3] bg_scaled=$bg_val rgb=$r,$g,$b bg=$calman_bg");
       &clean_pattern_files();
      ...
      if($type =~/RGB_S/) {
       ...
       &log("Calman RGB_S: rgb=$r,$g,$b size=$win_pct% bg=$calman_bg res=${w_s}x${h_s}");
       # full field pattern
       if($win_pct >= 100) {
