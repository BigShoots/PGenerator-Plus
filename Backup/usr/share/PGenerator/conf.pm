...
sub get_conf (@) {
 ...
 # End for RPI p4
 &get_pgenerator_conf();
 # When 10-bit renderer is available and max_bpc >= 10, enable 10-bit patterns.
 # pgeneratord_10bit uses DRM dumb buffers with XRGB2101010 (native 10-bit),
 # bypassing Mesa's vc4 driver which doesn't support 10-bit render targets.
 if (int($pgenerator_conf{'max_bpc'} || 8) >= 10 && -x "/usr/sbin/pgeneratord_10bit") {
  $bits_default = 10;
 }
}
